...
test.describe('End-to-End Test for Kiminoto App Test Plan', () => {
  ...
  test('Signup, Follow Test Plan, and Delete Account', async ({ page, request }) => {
    // ==========================================
    // 1. Sign up an account with dummy info
    // ==========================================
    await test.step('Sign up a dummy account', async () => {
      await page.goto('/signup');
      
      // Fill the registration form
      await page.getByLabel('Full Name').fill('Dummy Tester');
      await page.getByLabel('Email').fill('dummy.tester@example.com');
      await page.getByLabel('Password').fill('Password123!');
      await page.getByLabel('Telephone').fill('0991234567');
      ...
      // Wait for navigation after successful login/signup (e.g., redirect to home page)
      await page.waitForURL('**/');
    });

    // ==========================================
    // 2. Follow the test plan from the PDF file
    // ==========================================
    ...
    await test.step('TC1-2-1 & TC1-2-2: Upvote a review & Prevent duplicate upvotes', async () => {
      ...
      const firstReviewUpvoteBtn = page.locator('button:has(svg)').filter({ hasText: /^\s*$/ }).first(); 
    ...
    await test.step('TC1-4-1: Sort reviews for a car provider based on voting score (most popular)', async () => {
      ...
      const mostPopularBtn = page.getByRole('button', { name: /MOST POPULAR/i });
    ...
    await test.step('TC1-4-2: Sort reviews for a car provider based on most recent', async () => {
      ...
      const newestFirstBtn = page.getByRole('button', { name: /NEWEST FIRST/i });
    ...
    await test.step('Delete the dummy account', async () => {
      ...
      if (await deleteAccountBtn.isVisible()) {
        ...
        console.log('Delete Account button not found in UI. Fallback: API deletion not performed because backend is unavailable.');
